...
class IpcProvider():
    ...
    def post(self, path, payload=None, param=None):
        ...
        """
        try:    
            print("Inside POST request try statement now")
            with requests.session() as mysession:
                print("Inside with statement of POST request now")
                url = self._build_url(path)
                payload_data = json.dumps(payload)
                self.logger.info("API: " + url + ",data:" + payload_data)
                headers = {'Cookie': self._session_token}
                ...
                result = json.loads(response.text)
                print("Leaving POST request")
                return result
            print ("Leaving POST request body")
            return False
        except Exception as e:
            self.logger.error(e)
    ...
    def connect(self):
        ...
        with requests.session() as mysession:
            try:
                ...
                print("URL: " + url + " data: " + payload + "Sent from IPCprovider.connect")
